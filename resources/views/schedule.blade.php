                                ...
                                <span class="${task.completed ? 'line-through text-gray-500' : 'text-[#132C51]'} text-lg translate-y-[-2px] break-words whitespace-normal break-all">
                                    <div class="flex flex-col">
                                        <div class="flex flex-wrap items-center">
                                            ${task.title}
                                            ${task.priority ? `
                                                    <svg class="flex-shrink-0 inline-block w-6 h-6 ml-2 ${task.priority === 'Urgent' ? 'text-red-500' : (task.priority === 'High' ? 'text-yellow-500' : (task.priority === 'Normal' ? 'text-blue-500' : 'text-green-500'))}" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            fetch(`/tasks/${taskId}`)
                ...
                .then(task => {
                    ...
                    if (editDetailsBtn) {
                        ...
                        task.completed ? editDetailsBtn.classList.add('hidden') : editDetailsBtn.classList.remove(
                            'hidden');
                    }
                });
        };
