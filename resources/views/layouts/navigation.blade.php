<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <!-- Header Atas -->
                <header class="flex items-center justify-between w-full px-6 pt-6 lg:px-10">
                    <!-- Kiri: Logo & Sapaan -->
                    <div class="flex">
                    ...
                    <!-- Kanan: Tanggal, Search, Avatar -->
                    <div class="flex items-center space-x-4">
                        <div class="hidden font-semibold text-gray-700 md:block">
                        ...
                        <img class="object-cover w-12 h-12 rounded-full"
                            src="https_placehold.co/100x100/E2E8F0/334155?text={{ substr(Auth::user()->name, 0, 1) }}"
                            alt="User Avatar">
                    </div>
                </header>
            </div>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                onclick="event.preventDefault();
                                                if(confirm('Are you sure you want to logout?')) this.closest('form').submit();">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
